<?php
// Extract flex box inner settings from atts
$flex_direction = !empty($atts['flex_direction']) ? $atts['flex_direction'] : 'row';
$gap = !empty($atts['gap']) ? intval($atts['gap']) : 15;
...
// Build CSS classes
$css_classes = array('liwa-flex-box-inner', 'wpb_content_element', 'vc_element');
if (!empty($el_class)) {
...
if ($flex_direction === 'row') {
    $css_classes[] = 'liwa-flex-box-inner--row';
} elseif ($flex_direction === 'column') {
    $css_classes[] = 'liwa-flex-box-inner--column';
}

// Generate unique ID for styling
$unique_id = 'liwa-flex-box-inner-' . uniqid();

// Add container-specific data attributes
$container_data = ' data-element_type="liwa_flex_box_inner" data-vc-container=".liwa-flex-box-inner"';
